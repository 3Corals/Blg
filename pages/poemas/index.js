...
export default function Poemas({ posts, tags }) {
  ...
  function onSearch(e) {
    ...
    const url = val ? `/poemas?q=${e.target.value.toLowerCase()}` : '/poemas'
  ...
  return (
    <Fragment key={postsToShow.length ? 'non-empty' : 'empty'}>
      <Head>
        <title key="title">Poemas - B.Sánchez</title>
      ...
      <div className="blog-page-content">
        <div className="posts-box">
          <div className="blog-title">
            <h1>Poemas</h1>
            <div>{filteredPosts.length} poemas</div>
          </div>

          {postsToShow.map((post) => (
            <PostItem parent="poemas" key={post.slug} {...post} />
          ))}

          {pages > 1 && (
            <Pagination
              href={(p) => `/poemas?q=${query.q || ''}&page=${p}`}
